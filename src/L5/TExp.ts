...
export type AtomicTExp = NumTExp | BoolTExp | StrTExp | VoidTExp | NeverTExp | AnyTExp ; //new
...
export type CompoundTExp = ProcTExp | TupleTExp | UnionTExp | InterTExp | TypePredTExp;//new
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x) || isTypePredTExp(x); //new
...
export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | UnionTExp | InterTExp;//new
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isUnionTExp(x) || isInterTExp(x);//new
...
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp =>
    {
        if (equals(te1, te2) || isNeverTExp(te1) || isAnyTExp(te2)) {
        ...
        if (isUnionTExp(te1)) {
            return makeUnionTExp(te1.components.filter(comp => !isSubType(comp, te2)));
        }
        if (isInterTExp(te1)) {
            return makeInterTExp(te1.components.map(comp => makeDiffTExp(comp, te2)));
        }
        if (isInterTExp(te2)) {
            return makeUnionTExp(te2.components.map(comp => makeDiffTExp(te1, comp)));
        }
        if (isUnionTExp(te2)) {
            return makeInterTExp(te2.components.map(comp => makeDiffTExp(te1, comp)));
        }
        return te1;
    };
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    isNeverTExp(te1) ? true : //new
    isAnyTExp(te2) ? true : //new
    isAnyTExp(te1) && isTVar(te2) ? true : //new
    (isUnionTExp(te1) && isUnionTExp(te2)) ? isSubset(te1.components, te2.components) :
    isUnionTExp(te2) ? containsType(te2.components, te1) :
    (isInterTExp(te1) && isInterTExp(te2)) ? isSubset(te1.components, te2.components): // new
    ...
    isAtomicTExp(te1) ? te1.tag === te2.tag://new
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    ...
    (texps[0] === "is?") ? parseTypePredTExp(texps) ://new 
    parseProcTExp(texps);
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isAnyTExp(x) ? makeOk('any') : //new
        isNeverTExp(x) ? makeOk('never') : //new
        ...
        isInterTExp(x) ? mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) => //new
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isTypePredTExp(x) ? mapv(unparseTExp(x.type), (type : string) => 'is?' + type) : //new
