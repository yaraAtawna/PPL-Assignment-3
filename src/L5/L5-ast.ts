...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    ...
    const isTypePredicate = (rest[0] === ":" && rest[1] === "is?");
    let bodyStartIndex = 0;
    let returnTE: Result<TExp>;

    if (isTypePredicate) {
        returnTE = parseTExp(rest.slice(1, 3));
        bodyStartIndex = 3;
    } else if (rest[0] === ":") {
        returnTE = parseTExp(rest[1]);
        bodyStartIndex = 2;
    } else {
        returnTE = makeOk(makeFreshTVar());
        bodyStartIndex = 0;
    }

    const body = mapResult(parseL5CExp, rest.slice(bodyStartIndex));
