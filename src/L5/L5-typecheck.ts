...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType ,isTypePredTExp,makeDiffTExp ,isBoolTExp} from "./TExp";
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TEnv[]> => {
    ...
    const firstArg = e.rands[0];
    if (!isVarRef(firstArg)) {
        return makeFailure("Operand is not a variable reference");
    }

    return bind(typeofApp(e, tenv), (appType: TExp) =>
        bind(typeofExp(firstArg, tenv), (argType: TExp) => {
            if (isTypePredTExp(appType)) {
                const trueEnv = makeExtendTEnv([firstArg.var], [appType.type], tenv);
                const falseEnv = makeExtendTEnv([firstArg.var], [makeDiffTExp(argType, appType)], tenv);
                return makeOk([trueEnv, falseEnv]);
            } else {
                return makeFailure("Type of application is not a type predicate");
            }
        })
    );
}
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind(isTypePredApp(ifExp.test, tenv), (newEnvs: TEnv[]) =>
            bind(typeofExp(ifExp.then, newEnvs[0]), (thenTE: TExp) =>
                bind(typeofExp(ifExp.alt, newEnvs[1]), (altTE: TExp) =>
                    makeOk(makeUnion(thenTE, altTE)))
            )
        ),
        makeOk,
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> =>  { //new
    const argsTEs = map((vd) => vd.texp, proc.args);
    const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
    
    if (isTypePredTExp(proc.returnTE)) {
        if (proc.args.length !== 1) {
            return makeFailure(`Type predicate must take exactly 1 argument! Got ${proc.args.length}`);
        }
        const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) => 
                                isBoolTExp(body) ? makeOk(true) : bind(unparseTExp(body), (body: string) => 
                                    makeFailure(`Type of body must be bool for type predicate! Got type: ${body}`)));
